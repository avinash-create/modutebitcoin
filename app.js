...
async function fetchCrypto() {
    try {
        const responseBody = await fetch("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=10&page=1&sparkline=false");
        if (!responseBody.ok) {
...
searchBox.addEventListener("input", (e)=>{
    ...
    let val = arr.filter((ele,i)=>{
        return (ele.name.toLowerCase().includes(enteredText.toLowerCase()) || ele.symbol.toLowerCase().includes(enteredText.toLowerCase()));
    }
    );
    console.log(val);
    loopObject(val);
...
sortPercentage.addEventListener("click", (e)=>{
    const arr = JSON.parse(localStorage.getItem("coinData"));
    const sorted = arr.sort((a,b)=>a.price_change_percentage_24h - b.price_change_percentage_24h);
    // console.log(sorted);
    loopObject(sorted);
}
);
